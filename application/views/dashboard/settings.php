    ...
    <form class="stack-form" action="<?php echo html_escape(site_url('auth/settings')); ?>" method="post" enctype="multipart/form-data">
        ...
        <section class="form-section" aria-labelledby="access-heading">
            ...
            <div class="form-grid form-grid-spaced">
                <div class="field"><label for="min_password_length">Longitud mínima de contraseña</label><input id="min_password_length" name="min_password_length" type="number" value="<?php echo html_escape($value('min_password_length', '12')); ?>" min="8" max="128" step="1" inputmode="numeric" required><small>Entre 8 y 128 caracteres.</small></div>
                <div class="field"><label for="maximum_login_attempts">Intentos de acceso permitidos</label><input id="maximum_login_attempts" name="maximum_login_attempts" type="number" value="<?php echo html_escape($value('maximum_login_attempts', '3')); ?>" min="1" max="20" step="1" inputmode="numeric" required><small>Antes de bloquear temporalmente el acceso.</small></div>
                <div class="field"><label for="lockout_time">Bloqueo temporal (segundos)</label><input id="lockout_time" name="lockout_time" type="number" value="<?php echo html_escape($value('lockout_time', '600')); ?>" min="60" max="86400" step="1" inputmode="numeric" required></div>
            </div>
        </section>
        <section class="form-section" aria-labelledby="smtp-heading">
            ...
            <div class="form-grid">
                <div class="field"><label for="smtp_host">Servidor SMTP</label><input id="smtp_host" name="smtp_host" type="text" value="<?php echo html_escape($value('smtp_host')); ?>" maxlength="255" autocomplete="off" placeholder="smtp.ejemplo.com"></div>
                ...
                <div class="field"><label for="smtp_crypto">Seguridad</label><select id="smtp_crypto" name="smtp_crypto"><option value="tls" <?php echo $value('smtp_crypto', 'tls') === 'tls' ? 'selected' : ''; ?>>STARTTLS</option><option value="ssl" <?php echo $value('smtp_crypto') === 'ssl' ? 'selected' : ''; ?>>SSL/TLS</option><option value="" <?php echo $value('smtp_crypto') === '' ? 'selected' : ''; ?>>Ninguna</option></select></div>
                ...
                <div class="field"><label for="smtp_from_name">Nombre remitente</label><input id="smtp_from_name" name="smtp_from_name" type="text" value="<?php echo html_escape($value('smtp_from_name')); ?>" maxlength="100"></div>
            </div>
        </section>
        <section class="form-section" aria-labelledby="branding-heading">
            ...
            <div class="form-grid">
                <div class="field"><label for="site_name">Nombre de la aplicación</label><input id="site_name" name="site_name" type="text" value="<?php echo html_escape($value('site_name', 'CodeIgniter')); ?>" maxlength="100" required></div>
                ...
                <div class="field field-full"><label for="brand_logo_file">Logo</label><input id="brand_logo_file" name="brand_logo_file" type="file" accept="image/jpeg,image/png,image/webp"><small>JPEG, PNG o WebP. Máximo 2 MB y 2048 × 2048 píxeles.</small></div>
            </div>
        </section>
